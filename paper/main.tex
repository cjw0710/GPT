...
Anonymous code is available at \url{https://anonymous.4open.science/status/HARP-5D26}.
\end{abstract}
...
Their filtering interpretation follows graph signal processing and localized spectral convolution \cite{hammond2011wavelets,shuman2013emerging,defferrard2016convolutional,ortega2018graph}.
They are strong when connected nodes tend to share labels, but neighborhood aggregation often behaves as a low-pass filter that can over- or mis-smooth under heterophily \cite{nt2019revisiting,li2018deeper,oono2020graph,rong2020dropedge,chen2020simple,yan2022two}.
...
ACM, GloGNN, label-wise propagation, and neural sheaf diffusion broaden this space through adaptive channels, global aggregation, label-conditioned messages, and learned transport \cite{luan2022revisiting,li2022global,dai2022labelwise,bodnar2022neural}.
Spectral support and polynomial-basis analyses further motivate richer frequency responses \cite{balcilar2021breaking,wang2022spectral}.
...
Graph benchmarks are sensitive to split protocol, metric choice, and baseline coverage \cite{shchur2018pitfalls,pei2020geomgcn,lim2021large,platonov2023critical}.
...
\caption{\method{} at a glance. A fixed graph split feeds two structural experts trained with identical labels and budgets. Blue retains self-information; green separates the ego path. Their validation scores are compared by the frozen rule $a_E-a_H>1.96\,\mathrm{SE}$, after which one branch is locked and evaluated once on test labels.}
...
\IfFileExists{tables/critical_heterophily_dataset_stats.tex}{
  \input{tables/critical_heterophily_dataset_stats.tex}
}{
...
\IfFileExists{tables/harp_select_results.tex}{
  \input{tables/harp_select_results.tex}
}{
...
\IfFileExists{tables/harp_select_paired_tests.tex}{
  \input{tables/harp_select_paired_tests.tex}
}{
  \begin{tabular*}{0.94\linewidth}{@{\extracolsep{\fill}}llcccc@{}}
  \toprule
...
\IfFileExists{tables/harp_select_robust_tests.tex}{
  \input{tables/harp_select_robust_tests.tex}
}{
  \begin{tabular*}{0.94\linewidth}{@{\extracolsep{\fill}}llccccc@{}}
  \toprule
...
\IfFileExists{tables/critical_heterophily_external_main.tex}{
  \input{tables/critical_heterophily_external_main.tex}
}{
  \begin{tabular*}{0.94\linewidth}{@{\extracolsep{\fill}}llcccccc@{}}
  \toprule
...
\IfFileExists{tables/critical_heterophily_binary_complete_paired_tests.tex}{
  \input{tables/critical_heterophily_binary_complete_paired_tests.tex}
}{
...
\IfFileExists{tables/webkb_ablation_results.tex}{
  \input{tables/webkb_ablation_results.tex}
}{
  \begin{tabular*}{0.94\linewidth}{@{\extracolsep{\fill}}lccccc@{}}
  \toprule
...
\IfFileExists{tables/webkb_filter_weights.tex}{
  \input{tables/webkb_filter_weights.tex}
}{
...
Figure~\ref{fig:selector-audit} makes this audit split-level rather than aggregate.
The validation surplus is decisively positive on Squirrel, Chameleon, and Roman-Empire, matching their all-\esep{} routing, while Actor and Amazon-Ratings remain close to the decision boundary and are conservatively retained by \base.
The held-out panel confirms aligned gains in the three switched datasets while exposing borderline cases hidden by mean-only tables.
Figure~\ref{fig:selector-sensitivity} further shows that increasing the confidence multiplier monotonically reduces \esep{} selections; the frozen $z=1.96$ rule lies in a low-regret plateau rather than at a test-tuned optimum.

\begin{figure}[!b]
\centering
\IfFileExists{figures/selector_audit.pdf}{
  \includegraphics[width=0.87\linewidth]{figures/selector_audit.pdf}
}{
  \includegraphics[width=0.87\linewidth]{figures/selector_audit.png}
}
\caption{Split-level selector audit over eight accuracy datasets. Positive validation surplus triggers \esep; test advantage is inspected only after routing. Diamonds denote means.}
\label{fig:selector-audit}
\end{figure}

\begin{figure}[!b]
\centering
\IfFileExists{figures/selector_sensitivity.pdf}{
  \includegraphics[width=0.87\linewidth]{figures/selector_sensitivity.pdf}
}{
  \includegraphics[width=0.87\linewidth]{figures/selector_sensitivity.png}
}
\caption{Post-hoc sensitivity over 80 accuracy splits. The submitted $z=1.96$ point is frozen; the sweep is diagnostic only.}
\label{fig:selector-sensitivity}
\end{figure}

\section{Conclusion}

\method{} treats heterophilous graph learning as a validation-calibrated choice between residual polynomial experts rather than a commitment to one universal filter.
The self-loop expert is useful on WebKB; the ego-separated expert repairs Chameleon and Squirrel failures and improves Roman-Empire; the frozen selector routes without test labels.
Split-level audits distinguish decisive switches from borderline cases, while the threshold sweep shows that $z=1.96$ is a conservative low-regret operating point rather than a test-tuned optimum.
The evidence is deliberately bounded and supports this auditable specialist-and-routing formulation.
