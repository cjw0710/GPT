...
\IfFileExists{figures/selector_audit.pdf}{
  \includegraphics[width=\linewidth]{figures/selector_audit.pdf}
}{
  \includegraphics[width=\linewidth]{figures/selector_audit.png}
}
\caption{Selector audit over eight accuracy datasets (10 splits each). Positive validation surplus triggers \esep; the test difference is inspected only after routing. Blue retains \base, green selects \esep, and diamonds denote means.}
...
\IfFileExists{figures/selector_sensitivity.pdf}{
  \includegraphics[width=\linewidth]{figures/selector_sensitivity.pdf}
}{
  \includegraphics[width=\linewidth]{figures/selector_sensitivity.png}
}
\caption{Post-hoc sensitivity of the frozen selector over all 80 accuracy splits. The submitted operating point is $z=1.96$; the sweep is diagnostic and does not retune reported results.}
...
The self-loop expert is useful on WebKB, the ego-separated expert repairs Chameleon and Squirrel failures and improves Roman-Empire, and the frozen selector routes between them without test labels.
...
The evidence is deliberately bounded: it supports an auditable specialist-and-routing formulation, not universal leaderboard dominance.
Stronger official baselines, richer expert libraries, and cheaper selectors are the next tests, and should retain the same pre-test decision and paired-evidence protocol.
