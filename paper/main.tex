...
For the binary critical-heterophily suite, we implement ROC-AUC support and report the complete 10-split Minesweeper branch comparison; Tolokers and Questions are included only as loader and metric sanity checks and are excluded from the main empirical claims.

\begin{table}[t]
\centering
\caption{External critical-heterophily dataset statistics for split 0. Edges are undirected after symmetrization; Hom. is edge homophily.}
...
When a model changes the role of self-loops, high-pass residuals, gates, or branch selectors, the paper should show how that choice was made and where it fails.
...
The same discipline makes the result easier to falsify, extend, and compare against stronger baselines.

The most valuable additions are not cosmetic model complexity.
They are tighter external validation, stronger license-compatible baselines, and a cheaper selector that keeps the same auditability.
If those additions preserve the current pattern on Roman-Empire, Minesweeper, and the WebKB datasets while reducing the deficits on Chameleon and Squirrel, the work would support a stronger claim about branch-aware residual filtering.
If they do not, the same framework will still identify which structural assumption failed.
Either outcome is useful: the contribution is a reproducible way to ask whether a heterophilous graph needs self-preserving smoothing, ego-separated contrast, or neither.
This framing also makes the paper easier to build on.
New datasets can be added by logging the same validation quantities; new experts can be added by comparing their frozen branch regret; and new gates can be evaluated by whether they improve calibration as well as mean accuracy.
The resulting standard is modest but demanding.
A branch-aware graph model should not only fit labels on a benchmark.
It should explain which structural evidence it trusted, why that trust was justified before seeing the test labels, and where the same trust breaks down.
\method{} is a first step toward that standard for residual polynomial GNNs.
